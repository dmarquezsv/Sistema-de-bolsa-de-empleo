...
if (isset($_POST['datos'])) {
	...
	while ($item2=$stmt2->fetch()){
		...
		if ($item2['Tipo'] == "Contacto") {
			
			echo '<li><a class="text-body-color-dark media mb-15" href="chat?chats='.base64_encode($item2['idSolicitud']).'&solicitud='.base64_encode($item2['IDNotificacion']).'&estado='.base64_encode($item2['EstadoSolicitud']).'">
					<div class="ml-5 mr-15"><i class="fa fa-user text-success"></i></div>
					<div  class="media-body pr-10">
					<p class="mb-0">Tipo de mensaje: '.$item2['Tipo'].'</p><br>
			...
			';	
		}else if($item2['Tipo'] == "Solicitud-Pago"){
