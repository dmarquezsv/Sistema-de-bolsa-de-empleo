...
if (isset($_POST['datos'])) {
	...
	while ($item2=$stmt2->fetch()){
		...
		if ($item2['Tipo'] == "Contacto") {
			...
			echo '<li><a class="text-body-color-dark media mb-15" href="chat?chats='.base64_encode($item2['idSolicitud']).'&solicitud='.base64_encode($item2['IDNotificacion']).'&estado='.base64_encode($item2['EstadoSolicitud']).'&reclutador=notificar">
			...
			';	
		}else if($item2['Tipo'] == "Actualizar CV"){
