...
if ($resultPerfilEmpresa == 0) {
	...
	echo "<script>location.href='crear-perfil-empresa.php?notificar=1';</script>";
	

}else if($resultPerfilEmpresa == 1){

	$sqlEspecial="SELECT IDEmpresa,Nombre,`estado` , `Expira` , `fecha_registro` ,`logo` FROM `empresa_perfil` WHERE `IDUsuario` = ?";
	...
	if ($estado == "Vencido") {
		...
		echo "<script>location.href='cpanel-visitante?success=0';</script>";
	}
	else if ($Expira <= $fechaActual) {

		$sqlEmpresa = "UPDATE `empresa_perfil` SET `estado` = 'Vencido' WHERE `IDEmpresa` = :IDEmpresa ";
		$stmtSeguridad =  Conexion::conectar()->prepare($sqlEmpresa);
		$stmtSeguridad->bindParam('IDEmpresa', $IDEmpresa , PDO::PARAM_STR);
