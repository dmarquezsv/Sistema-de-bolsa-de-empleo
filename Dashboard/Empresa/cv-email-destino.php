...
$sql9="SELECT `IDEmpresa` FROM `empresa_perfil` WHERE `IDUsuario` = ?";
$stmt9 =  Conexion::conectar()->prepare($sql9);
$stmt9->execute(array($IDUser));
while ($item=$stmt9->fetch()){

    $IDEmpresa = $item['IDEmpresa']; // resultado del contado de visitas
}

//vERIFICA SI EXISTE UN REPORTE CON LA FECHA ACTUAL SI ES CERO CREA EL  REPORTE  SI ES 1 INCREMENTA CONTADOR DE VISITAS
$sql18="SELECT COUNT(`IDReporte`) AS 'FechaDelDia' FROM `reportes_generales` WHERE `fecha` = ? AND `IDEmpresa` = ? AND `Tipo` = 'Curriculum Vitae enviados por e-mail'";
...
if($ResultReporteDelDia == 0){
	...
	$sql19="INSERT INTO `reportes_generales` (`IDEmpresa`, `Tipo`, `contador`, `fecha`) VALUES (:IDEmpresa, 'Curriculum Vitae enviados por e-mail', '1', :fecha)";
  ...
  //Realizamos una consulta para buscar  el conteo  con su respectivo fecha y empresa. La opcion sera perfiles vistos por dia
	$sql20="SELECT `contador` FROM reportes_generales WHERE `fecha` = ?  AND `IDEmpresa` = ? AND `Tipo` = 'Curriculum Vitae enviados por e-mail'";
...
//Actualizamos la visita del contador
 $sql19="UPDATE `reportes_generales` SET `contador`= :contador WHERE `fecha` = :fecha  AND `IDEmpresa` = :IDEmpresa AND `Tipo` = 'Curriculum Vitae enviados por e-mail'";
 $stmt19 =  Conexion::conectar()->prepare($sql19);
   ...
   //file_put_contents($filename, $pdf);
	//$dompdf->stream($filename);
			...
			try {
			    ...
			    <center>
			    <a href="https://mundoempleosca.com/Dashboard/Empresa/cv?id='.base64_encode($IDUser).'" target="_blank" rel="noopener noreferrer" data-auth="NotApplicable" style="display:inline-block; min-width:250px; font-family:Tahoma,Arial,sans-serif; font-size:18px; font-weight:bold; color:#0B3486; line-height:50px; text-align:center; text-decoration:none; background-color:#FCC201; border-radius:50px; padding:16px 24px; line-height:1">Ver Perfil</a>
			    </center>
