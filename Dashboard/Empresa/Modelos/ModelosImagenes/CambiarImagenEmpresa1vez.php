...
if (isset($_POST['Guardar'])) {
	session_start(); 
	$IDUser = $FuncionesApp->test_input($_POST['IDempresa']);	
	$imgNombre = $FuncionesApp->test_input($_POST['imgempresa']);
	...
	if($_FILES["imgusu"]["error"]>0){
		...
		if($imgNombre != "pordefecto.jpg")
		{
			$destino = "../../../../main/img/LogosEmpresas/".$imgNombre;
			unlink($destino);
		}
		...
		if(in_array($_FILES["imgusu"]["type"], $tipoimg)){
			...
			$rutaimg = '../../../../main/img/LogosEmpresas/';
			...
			if(!file_exists($nombreimg)){               
				...
				$sql = "UPDATE `empresa_perfil` SET `logo` = :logo WHERE `IDEmpresa` = :IDEmpresa";
				$stmt =  Conexion::conectar()->prepare($sql);
				$stmt->bindParam(':logo', $img , PDO::PARAM_STR);
				$stmt->bindParam(':IDEmpresa', $IDUser , PDO::PARAM_STR);
