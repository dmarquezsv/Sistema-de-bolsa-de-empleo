<?php 
		...
		$iduser = $FuncionesApp->test_input(base64_decode($_GET['id'])); 
		$email = $FuncionesApp->test_input($correo); 
			
			
		$sql = "SELECT Estado FROM usuarios_cuentas WHERE IDUsuario = ? AND Correo = ?";
		$respuesta = Conexion::conectar()->prepare($sql);
		$respuesta->execute(array($iduser ,$email ));
		...
		if (!$respuesta->execute()) {
			...
			if($Estado =="Seguridad"){
				...
				$sql ="UPDATE `usuarios_cuentas` SET `Estado` = 'Activo' WHERE IDUsuario = ?";
				$respuesta2 = Conexion::conectar()->prepare($sql);
				$respuesta2->execute(array($iduser));
